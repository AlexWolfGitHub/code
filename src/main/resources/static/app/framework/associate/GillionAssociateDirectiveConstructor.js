...
define('framework/associate/GillionAssociateDirectiveConstructor', ['angular', 'artTmpl', 'underscore', 'config.properties',
    ...
    return function ($window, $document, $compile, $dataSourceManager, $parse, $timeout, $q, $filter, $tabindex, GillionLocationService, GillionMsgService, $rootScope, AssociatePromiseService, GillionMsg, AssociateSettingService) {
        ...
        Associate.prototype.blurHandle = function () {
            ...
            //if (associate.attrs.multi === true || associate.attrs.multi === 'true') associate.emptyOnSelect();
            this.finishLoading();
        };
        ...
        Associate.prototype._bindEvents = function () {
            ...
            $(window.parent.document).on('mousedown.associate' + scope.$id, function (event) {
                ...
                if (($input.is(target) && $input[0] != document.activeElement) || $table.is(target) || $table.find(target).length) {
                    if (scope.tableShow) {
                        //me.blurHandle();
                        me.pageMouseDown();
                    }
            ...
            $document.on('focus.associate' + scope.$id, '*', function (event) {
                ...
                var $input = $(scope.inputText);
                var $table = scope.$tableWrapper;
                if (!($input.is(target) || $table.is(target) || $table.find(target).length)) {
                    if (scope.tableShow) {
                        //me.apply();
                        me.hideView();
                    }
        ...
        Associate.prototype._rowClick = function (event, $target, $row) {
            var me = this,
                scope = me.scope,
        ...
        Associate.prototype._render = function () {
            ...
            var pagerWidth = $tableWrapper.find(".form-suggestbox-pager").width();
            ...
            if ($tableWrapper.css("display") == "none") {
                ...
                pagerWidth = $tableWrapper.children("div.form-suggestbox-pager").width();
        ...
        AssociateMulti = (function (superClass) {
            ...
            AssociateMulti.prototype._rowClick = function ($event, $target, $row) {
                var me = this,
                    scope = me.scope,
            ...
            AssociateMulti.prototype.showSelected = function () {
                ...
                me.selectedRowToModelValue();
                if (missKeyArr.length > 0 && me.scope.unmatchRemove && (missKeyArr.length == 1 && missKeyArr[0] != "")) {
        ...
        return {
            ...
            compile: function (tElement, tAttrs, transclude) {
                ...
                return function (scope, element, attrs, controllers) {
                    ...
                    function showHandle() {
                        if (angular.isDefined(element.attr('disabled')) || element.hasClass('disabled') || angular.isDefined(element.attr('readonly')) || element.hasClass('readonly')) return;
                    ...
                    function readonlySetting() {
                        $timeout(function () {
                            if (scope.ngDisabled || scope.disabled || scope.ngReadonly || scope.readonly) scope.inputText.attr("readonly", "readonly");
