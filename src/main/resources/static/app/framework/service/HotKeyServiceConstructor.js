define('framework/service/HotKeyServiceConstructor', ['angular', 'underscore'], function (angular, _) {
    ...
    Proto._addedEvent = function (fieldEl, $el) {
        var me = this;
        if ($el == undefined) $el = me._getHandleElement(fieldEl);
    ...
    Proto._listen = function () {
        ...
        if (win != window) {
            win.$(win).on("keydown.hotkey", function (event) {
                func(event, win.document);
            });

            win.$("iframe").each(function (index, iframe) {
                if (iframe.contentWindow == window) {
                    win.$(iframe).on("keydown.hotkey", function (event) {
                        func(event, iframe.contentWindow.document);
                    });
                }
            })
        }
        angular.element(window).on("keydown.hotkey", function (event) {
            func(event, document);
        });
        ...
        function func(event, doc) {
            ...
            if (keyCode == 13) {
                ...
                if (doc.activeElement.tagName == "INPUT") {
                    if (doc.activeElement.hasAttribute("enter-index")) index = doc.activeElement.getAttribute("enter-index");
