    ...
    return angular.module('GillionMsgModule', [])
        ...
        .factory('GillionMsg', ['$timeout', '$window', '$rootScope', 'GillionMsgService', 'ZIndex', 'GillionLocationService', function ($timeout, $window, $rootScope, GillionMsgService, ZIndex, GillionLocationService) {
            ...
            function buildButton(opts, owner) {
                ...
                $btn.append(opts.text).on('click', function () {
                    var flag;
            ...
            function MsgBox(opts) {
                ...
                this.$ele.appendTo(getMainWindow(window).document.body);
            }
            ...
            MsgBox.prototype.setHeight = function (val) {
                var $ele = this.$ele,
                    ...
                    $body = $ele.find('.modal-body'),
                    $footer = $ele.find('.modal-footer');
                $body.height(val - $header.outerHeight() - $footer.outerHeight() - 40);
            };
            /**
             * 设置标题
            ...
            MsgBox.prototype.show = function () {
                ...
                if ($mask) $mask.show();
                $ele.show();
                mainJq(getMainWindow(window).document.body).css('overflow', 'auto');
            };
            ...
            MsgBox.prototype.close = function () {
                ...
                $mask && $mask.remove();
                mainJq(getMainWindow(window).document).off('focus.mask', '*');
                if (getMainWindow(window) != window) {
            ...
            MsgBox.prototype.render = function () {
                var $ele = this.$ele,
                    opts = this.opts,
                    mainWindow = mainJq(getMainWindow(window)),
                    clientHeight, top;

                opts.modal && this.mask();
                //强制居中布局
                if (getMainWindow(window).frameElement) {
                    clientHeight = getMainWindow(window).frameElement.ownerDocument.documentElement.clientHeight;
                ...
                $header.on('mousedown', function (e) {
                    ...
                    function mouseMove(e) {
                        if (getMainWindow(window).frameElement) {
                            ...
                            maxTop = getMainWindow(window).frameElement.ownerDocument.documentElement.clientHeight -
