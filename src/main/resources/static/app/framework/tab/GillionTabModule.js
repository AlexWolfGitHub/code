...
define('framework/tab/GillionTabModule', [
     ...
     * @constructor
     */
    function TabPanel(element, options, Permissions) {
    ...
    TabPanel.prototype.bindEvents = function () {
        ...
        $navs.on('click', '.fi-refresh-small', function () {
            ...
            options.onSelect({title: $currNav.data('title'), url: $currNav.data('url')});
        });
    };
    ...
    TabPanel.prototype.loadTab = function (key) {
        ...
        var onLoadStr = 'onload="(function(){var tIframe=setInterval(function(){var h=this.contentWindow.document.body.scrollHeight;if(h>300){clearInterval(tIframe);}this.style.height=(h+10)+\'px\';}.bind(this), 1000)}.bind(this))();"';
        if (this.$panel.hasClass('elastic-tabs' || this.$panel.parent().hasClass('elastic-tabs'))) {
            onLoadStr = '';
        }
        if (tab.data('url')) {
            tab.html('<iframe frameborder="0" src="' + urlRewrite(tab.data('url')) + '"' + onLoadStr + '></iframe>');
            tab.data('load', true);
    ...
    }]).directive('gTabPanel', ['$parse', 'Permissions', function ($parse, Permissions) {
        return {
            restrict: 'E',
            transclude: true,
            require: 'gTabPanel',
            ...
            controller: function ($scope, $element, $attrs) {
                ...
                tabPanel = new TabPanel($element, options, Permissions);
