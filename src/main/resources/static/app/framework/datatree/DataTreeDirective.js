define('framework/datatree/DataTreeDirective', ["angular", "underscore"], function (angular, _) {
        ...
        DataTreeProto.append = function (data, index) {
            ...
            if (me.isLazy()) {
                ...
                var el = node.getEl();
                if (parentNodeData) {
                    if (!parentNodeData["expanded"]) el.style.display = "node";
        ...
        TreeNodeProto.getEl = function () {
            ...
            if (nodeData.expanded) node.setAttribute("open", "true");
            ...
            folderIcon.onclick = function ($event) {
                ...
                if (!treeScope.disabled && nodeData.type === "branch") {
                    nodeData = dataTree.$treeHelper.searchNodeByTd(treeScope.treeData, nodeData.idProp);
                    if (dataTree.isLazy()) {
                    ...
                    if (!nodeData.expanded) {
                        if (beforeExpand === undefined || !(beforeExpand(paramArg) === false)) {
                            if (treeScope.async && _.isEmpty(nodeData.children)) {
                                nodeData.expanded = true;
        ...
        TreeNodeProto.nodeClick = function ($event) {
            ...
            var item = dataTree.$treeHelper.searchNodeByTd(treeScope.treeData, me.data.idProp);
            selectParams = {
                $event: $event,
                record: item.originData
            };
        ...
        TreeNodeProto.expandNode = function ($event, nodeData) {
            ...
            if (dataTree.isLazy()) {
                ...
                dataTree.$body.find(".tree-item").each(function (i, el) {
                    ...
                    //path.toString().indexOf(nodeData.path) === 0
                    if (nodeData.path != path && containPath(path, nodeData.path)) {
        ...
        TreeNodeProto.chooseItem = function ($event, item) {
            ...
            if (dataTree.canCheck(item)) {
                if (beforeCheck === undefined || beforeCheck(eventParams) !== false) {
                    item = dataTree.treeHelper.searchNodeByTd(treeScope.treeData, item.idProp);
                    ...
                    if (!treeScope.radio) {
                        dataTree.$body.find(".tree-item").each(function (i, el) {
                            ...
                            var nodeData = dataTree.treeHelper.searchNodeByTd(treeScope.treeData, id);
                            if (nodeData.checked)
                                box.attr("selected", "selected");
                            else
                                box.removeAttr("defaultSelected").removeAttr("selected");
