define('framework/dropdown/GillionDropDownDirectiveConstructor', ['angular', 'underscore', 'config.properties'], function (angular, _, config) {
    return function ($parse, $timeout, $document, $compile, $tabindex, GillionLocationService, GillionMsgService) {
        ...
        return {
            ...
            template: '<div ng-cloak class="form-dorpdown" ng-class="{\'is-show-menu\': isShow}" verify-target><input type="text" ondragstart="return false" ng-class="{\'is-show-menu\': isShow}" verify-target class="form-text" ng-disabled="ngDisabled || disabled || unfilter || readonly || ngReadonly" ng-model="display" value="请选择" >' +
            '<a href="javascript:void(0)" type="button" ng-click="showDropDown()" class="btn dropdown-toggle"> <i class="fi fi-caret"></i></a>' +
            '</div>',
            restrict: 'E',
                        ...
                        var tagSource = _.chain(tElement.context.children)
                            ...
                            .map(function (node) {
                                ...
                                if ($node.attr('item')) {
                                    ...
                                    item = {};
                                    item[scope.displayExpress] = $node.attr('text') || $node.text();
                                    item[scope.valueProp] = $node.attr('value');
                                }
                                return item;
                            })
                            .value();
