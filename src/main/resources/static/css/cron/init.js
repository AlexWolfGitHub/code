...
function initCheckBox(divid, start, cnt) {
    if (cnt != null && cnt > 0) {
        for (dc = start; dc <= cnt; dc++) {
            $("#" + divid).append(" <div class='col-xs-2 col-sm-2 col-md-2'> <label><input type='checkbox' value=" + dc + "> &nbsp;" + dc + "</label></div>");
        }
    }
}

function resetting(){
    $("input[class='numberspinner'][name='second']").val(0);
    $("input[class='numberspinner'][name='min']").val(0);
    $("input[class='numberspinner'][name='hour']").val(0);
    $("input[class='numberspinner'][name='day']").val(1);
    $("input[class='numberspinner'][name='month']").val(1);
    $("input[class='numberspinner'][name='week']").val(1);
    $("input[class='numberspinner'][name='year']").val(2018);
    clearCheckBox("l_second");
    clearCheckBox("l_min");
    clearCheckBox("l_hour");
    clearCheckBox("l_day");
    clearCheckBox("l_month");
    clearCheckBox("l_quarter");
    clearCheckBox("l_week");
    inition();
}
